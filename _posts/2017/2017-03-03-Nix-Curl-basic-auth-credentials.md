...
I can give the credentials to `curl` so that it can plug them in as it makes the
request using the `-u` (or `--user`) flag:
...
If I don't want the password showing up in my command-line history, I can just
provide the username and `curl` will prompt me for my password:
